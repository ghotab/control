...
/** true si el "Nivel" del usuario corresponde a perfil auxiliar/técnico/coordinador. */
...
/** true si el "Nivel" del usuario corresponde a jefe/gerente (ve todas las bases). */
export function esPerfilJefe(usuario) {
  if (!usuario) return false;
  const nivel = String(usuario[CAMPO_NIVEL] ?? "").trim().toLowerCase();
  return NIVELES_JEFE.some((n) => n.toLowerCase() === nivel);
}
