...
/** Combina pendientes + terminadas de Soporte Técnico en una sola lista,
 * cruzando cada fila con Monitoreo (por ID Ej. Flujo) para deducir el video. */
export function calcularListaCompacta(datosPendientes, datosTerminadas, datosMonitoreo) {
  const indiceMON = {};
  ...
  return [...marcar(datosPendientes, "Pendiente"), ...marcar(datosTerminadas, "Terminada")];
}
  ...
  async function procesarYMostrar(registros) {
    try {
      ...
      actualizarDatosWF({
        pendientes: filas,
        terminadas: [],
        lista: calcularListaCompacta(filas, [], monitoreoAcumulado),
      });
    } catch (err) {
      console.error(err);
...
function montarVistaJefe(root) {
  ...
  /** Publica pendientes + terminadas (con Monitoreo ya cruzado) para que
   * "Indicadores" y "Pendientes" en el sidebar los vean sin cargar nada de nuevo. */
  function publicarEnStore() {
    const pendientes = estado.pendientes.filas || [];
    const terminadas = estado.terminadas.filas || [];
    actualizarDatosWF({
      pendientes,
      terminadas,
      lista: calcularListaCompacta(pendientes, terminadas, monitoreoAcumulado),
    });
  }
}
