...
const COLUMNAS = [
  { key: "noEconomico", label: "No. Económico", mono: true },
  { key: "clave", label: "Clave", mono: true },
  { key: "nombre", label: "Nombre" },
  { key: "codigoAccidente", label: "Código del Accidente", mono: true },
  { key: "idEjFlujo", label: "ID Ej. Flujo", mono: true },
  { key: "fechaReporte", label: "Fecha del reporte", mono: true },
  { key: "fechaIncidente", label: "Fecha del incidente", mono: true },
  { key: "horarioIncidente", label: "Horario del incidente", mono: true },
  { key: "tramoLugar", label: "Tramo o lugar del accidente", wrap: true },
  { key: "hechosRelato", label: "Hechos - relato del conductor", wrap: true },
  { key: "observacionesSopTec", label: "Observaciones para Sop.Tec", wrap: true },
  { key: "diasTranscurridos", label: "Días transcurridos", mono: true },
  { key: "discoVigente", label: "Disco grabado" },
  { key: "autobusActualDisco", label: "Autobús donde está el disco", mono: true },
];
...
export function montarPendientesWF(root, { usuario, esAuxiliar }) {
      ...
      <div class="panel">
        ...
        <div class="panel-body">
          <div class="locked-note">
            La carga de archivos de Pendientes WF está disponible solo para el perfil
            <strong>auxiliar</strong>. Tu perfil actual no tiene esta opción habilitada.
          </div>
        </div>
      </div>`;
    return;
  ...
  function render() {
    root.innerHTML = `
      <div class="panel">
        ...
        <div class="panel-body">
          ${dropzoneHtml("input-principal", archivoPrincipal)}
  ...
  function dropzoneHtml(inputId, file) {
    return `
      <label class="dropzone" for="${inputId}">
        <input id="${inputId}" type="file" accept=".xlsx" />
        <strong>${file ? esc(file.name) : "Haz clic para elegir el archivo .xlsx"}</strong>
        <div>${file ? "Selecciona otro archivo para reemplazarlo" : "Exportación de Workflow con la bitácora del accidente"}</div>
      </label>`;
  }
  ...
  function bloqueSiniestroHtml() {
    return `
      ...
      <div style="margin-top:12px;">
        ${dropzoneHtml("input-siniestro", archivoSiniestro)}
      </div>`;
  }
    ...
    return `
      ...
      <div class="panel">
        ...
        <div class="panel-body" style="padding:0;">
          ${filas.length === 0 ? `<div class="empty-state">No hay pendientes de tu base en este archivo.</div>` : tablaHtml(filas)}
        </div>
      </div>
    `;
  }
        ...
        <table class="data">
          ...
          <tbody>
            ${filas
              ...
              <tr>
                ${COLUMNAS.map((c) => {
                  if (c.key === "diasTranscurridos") return `<td>${badgeDias(f.diasTranscurridos)}</td>`;
                  if (c.key === "discoVigente")
                    return `<td>${f.discoVigente ? `<span class="badge muted">${esc(f.discoVigente)}</span>` : `<span class="cell-muted">No disponible</span>`}</td>`;
                  const val = f[c.key];
                  const cls = [c.mono ? "tabular" : "", c.wrap ? "cell-wrap" : ""].filter(Boolean).join(" ");
                  return `<td class="${cls}">${val ? esc(val) : `<span class="cell-muted">—</span>`}</td>`;
                }).join("")}
              </tr>`
              )
              .join("")}
