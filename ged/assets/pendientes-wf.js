...
export function montarPendientesWF(root, { usuario, esAuxiliar }) {
  ...
  async function procesarYMostrar(registros) {
    try {
      ...
      const [mapaBases, mapaAliasBase] = await Promise.all([
        obtenerBasesPorAutobus(numerosUnicos),
        obtenerMapaAliasBase(),
      ]);
      ...
      const propiosDeBase = conAutobus.filter((r) => {
        ...
        const baseNorm = normalizarTextoBase(baseRaw);
        const baseTraducida = mapaAliasBase[baseNorm] || baseNorm;
        return baseTraducida === baseUsuarioNorm;
      });
