...
// Acumula lo que se vaya encontrando de la hoja de Monitoreo en cualquiera de
// los archivos que se suban (pendientes, terminadas o el de Registro del
// Siniestro) — no importa en cuál venga, se junta todo por "ID Ej. Flujo".
let monitoreoAcumulado = [];

function acumularMonitoreo(workbook) {
  try {
    const { registros } = extraerRegistros(workbook, HOJA_MONITOREO_CANDIDATOS);
    if (!registros || registros.length === 0) return;
    const indice = {};
    monitoreoAcumulado.forEach((r) => { indice[r.idEjFlujo] = r; });
    registros.forEach((r) => { indice[r.idEjFlujo] = r; });
    monitoreoAcumulado = Object.values(indice);
  } catch {
    // La hoja de Monitoreo es opcional: si no está en este archivo, seguimos sin ella.
  }
}
...
const CAMPOS = {
  ...
  clave: (k) => k === "clave",
  nombre: (k) => k === "nombre",
  nombreEntidad: (k) => k === "nombreentidad",
  ...
  observacionesAbordo: (k) => k === "observacionesparapersonalabordo",
  fechaInicio: (k) => k === "fechainicio",
  fechaFin: (k) => k === "fechafin",
  enLinea: (k) => k === "elautobusestaenlinea",
};

/** Detecta dinámicamente columnas "Link video 1", "Link video 2", ... "Link
 * video 30" (Soporte Técnico solo trae 3; Monitoreo puede traer hasta 30). */
function encontrarColumnasVideo(filaEncabezado) {
  const columnas = [];
  filaEncabezado.forEach((h, idx) => {
    if (/^linkvideo\d+$/.test(compactKey(h))) columnas.push(idx);
  });
  return columnas;
}
...
function extraerRegistros(workbook, candidatosHoja) {
  ...
  const columnasVideo = encontrarColumnasVideo(filas[idxEncabezado]);
  const registros = [];
  ...
  for (let i = idxEncabezado + 1; i < filas.length; i++) {
    ...
    registros.push({
      ...
      nombreEntidad: textoCelda(fila[mapa.nombreEntidad]),
      ...
      observacionesAbordo: textoCelda(fila[mapa.observacionesAbordo]),
      videos: columnasVideo.map((idx) => textoCelda(fila[idx])).filter(Boolean),
      fechaInicio: textoCelda(fila[mapa.fechaInicio]),
      fechaFin: textoCelda(fila[mapa.fechaFin]),
      enLinea: textoCelda(fila[mapa.enLinea]),
    });
  }
...
export function severidad(dias) {
...
// ============================================================================
// "¿Hay video?" y "Área de atención"
// ============================================================================

/** Deduce, a partir del texto libre de "Observaciones para personal abordo",
 * si todo indica que el video ya se compartió/envió. Es una aproximación de
 * lo que antes se decidía a ojo en Excel — ajusta PALABRAS_VIDEO_COMPARTIDO
 * en config.js si hace falta afinarla. */
function pareceVideoCompartido(texto) {
  const t = quitarAcentos(String(texto || "")).toLowerCase();
  if (!t) return false;
  return PALABRAS_VIDEO_COMPARTIDO.some((palabra) => t.includes(palabra));
}

/** "SI"/"SÍ" (con o sin acento/mayúsculas) en la columna "¿El autobús está
 * en línea?" de la hoja de Monitoreo. */
function esRespuestaAfirmativa(v) {
  const t = quitarAcentos(textoCelda(v)).trim().toLowerCase();
  return t === "si" || t === "s" || t === "x" || t === "verdadero" || t === "true";
}

/** registroST: la fila de Soporte Técnico (pendiente o terminada).
 * registroMON: su contraparte en Monitoreo (mismo ID Ej. Flujo), si existe.
 * Además de decidir "¿hay video?" y el área, guarda de qué hoja salieron los
 * links y la observación ganadores, para poder mostrarlos en el detalle. */
function calcularVideoYArea(registroST, registroMON) {
  const tabListo = registroST && registroST.videos?.length > 0 && pareceVideoCompartido(registroST.observacionesAbordo);
  if (tabListo) {
    return {
      hayVideo: "Sí",
      areaAtencion: "TAB",
      videosDetalle: registroST.videos,
      observacionesDetalle: registroST.observacionesAbordo,
    };
  }
  ...
  if (monListo) {
    return {
      ...
      videosDetalle: registroMON.videos,
      observacionesDetalle: registroMON.observacionesAbordo,
    };
  }

  return { hayVideo: "No", areaAtencion: "—", videosDetalle: [], observacionesDetalle: "" };
}

/**
 * "Segundo tipo de pendientes": el Centro de Monitoreo contestó que el
 * autobús SÍ está en línea — por lo que Workflow cierra la tarea sola y
 * Soporte Técnico jamás la ve — pero en realidad no cargó ningún video. Esas
 * son pendientes reales que nadie está seguimiento. De paso, las que SÍ
 * quedaron bien (con video) se reportan como Terminada/MONITOREO, para tener
 * el panorama completo.
 *
 * `idsCubiertos` son los ID Ej. Flujo que ya vienen de Soporte Técnico (para
 * no duplicarlos aquí — en teoría nunca deberían cruzarse, porque solo pasan
 * a Soporte Técnico cuando Monitoreo contestó "NO", pero por si acaso).
 */
async function construirUniversoMonitoreo(datosMonitoreo, idsCubiertos) {
  const candidatos = (datosMonitoreo || []).filter(
    (r) => r.noEconomico && esRespuestaAfirmativa(r.enLinea) && !idsCubiertos.has(r.idEjFlujo)
  );
  if (candidatos.length === 0) return { pendientes: [], terminadas: [] };

  const { filas } = await enriquecerRegistros(candidatos);
  ...
  filas.forEach((f) => {
    ...
    const base = {
      ...f,
      areaAtencion: "MONITOREO",
      videosDetalle: f.videos || [],
      observacionesDetalle: f.observacionesAbordo || "",
    };
    if (tieneVideo) terminadas.push({ ...base, estatus: "Terminada", hayVideo: "Sí" });
    else pendientes.push({ ...base, estatus: "Pendiente", hayVideo: "No" });
  });

  return { pendientes, terminadas };
}

/** Combina pendientes + terminadas de Soporte Técnico con el "segundo tipo"
 * de pendientes que solo vive en la hoja de Monitoreo (ver
 * construirUniversoMonitoreo), en una sola lista ya lista para mostrar. */
export async function calcularListaCompacta(datosPendientes, datosTerminadas, datosMonitoreo) {
  const indiceMON = {};
  (datosMonitoreo || []).forEach((r) => { indiceMON[r.idEjFlujo] = r; });

  const marcar = (arr, estatus) =>
    (arr || []).map((r) => {
      const mon = indiceMON[r.idEjFlujo];
      const { hayVideo, areaAtencion, videosDetalle, observacionesDetalle } = calcularVideoYArea(r, mon);
      return { ...r, estatus, hayVideo, areaAtencion, videosDetalle, observacionesDetalle };
    });

  const pendientesST = marcar(datosPendientes, "Pendiente");
  const terminadasST = marcar(datosTerminadas, "Terminada");

  const idsCubiertos = new Set([...pendientesST, ...terminadasST].map((r) => r.idEjFlujo));
  const { pendientes: pendientesMon, terminadas: terminadasMon } = await construirUniversoMonitoreo(datosMonitoreo, idsCubiertos);

  return {
    pendientes: [...pendientesST, ...pendientesMon],
    terminadas: [...terminadasST, ...terminadasMon],
  };
}
...
function montarVistaBase(root, baseUsuario) {
  ...
  function render() {
    try {
      ...
      console.error("Error al construir la vista de Pendientes WF:", err);
      root.innerHTML = `<div class="callout error">Ocurrió un error mostrando los resultados: ${esc(err.message || String(err))}. Los datos sí se procesaron — revisa "Indicadores" o "Pendientes" en el menú, y avisa para corregir esta vista.</div>`;
    }
  }
    ...
    return `
      <div class="subpaso-alerta">
        <div class="subpaso-alerta-head">
          <div class="subpaso-alerta-icon">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M12 8v5M12 16.5h.01" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
              <path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0Z" stroke="currentColor" stroke-width="1.7" stroke-linejoin="round"/>
            </svg>
          </div>
          <div>
            <h4>Paso obligatorio · falta completar estos pendientes</h4>
            <p>
              Tu archivo trae tareas <strong>pendientes</strong> sin datos capturados todavía (es normal:
              Workflow no llena esos campos hasta que la tarea del "Registro del Siniestro" se termina).
              Para completarlos, filtra en Workflow las <strong>tareas terminadas del flujo "Registro del
              Siniestro"</strong>, expórtalas y cárgalas aquí abajo.
            </p>
          </div>
        </div>
        ${dropzoneHtml(
  ...
  async function procesarYMostrar(registros) {
    try {
      ...
      const combinado = await calcularListaCompacta(filas, [], monitoreoAcumulado);
      // Los "pendientes tipo 2" (Monitoreo dijo que sí está en línea pero sin
      // video) todavía no están filtrados por base — se filtran aquí igual
      // que el resto, para que cada quien vea solo lo de su base.
      const pendientesFinal = combinado.pendientes.filter((f) => f.base && normalizarTextoBase(f.base) === baseUsuarioNorm);
      const terminadasFinal = combinado.terminadas.filter((f) => f.base && normalizarTextoBase(f.base) === baseUsuarioNorm);
      const tipo2 = pendientesFinal.length - filas.length;

      filasResultado = pendientesFinal;
      const partes = [];
      partes.push(`${filas.length} pendiente${filas.length === 1 ? "" : "s"} de Soporte Técnico`);
      if (tipo2 > 0) partes.push(`${tipo2} más detectados vía Monitoreo (dijo "en línea" pero sin video)`);
      ...
      actualizarDatosWF({
        ...
        terminadas: terminadasFinal,
        lista: [...pendientesFinal, ...terminadasFinal],
      });
    } catch (err) {
      console.error(err);
...
function montarVistaJefe(root) {
  ...
  function render() {
    ...
    try {
      ...
      console.error("Error al construir la vista de Pendientes WF (jefe):", err);
      root.innerHTML = `<div class="callout error">Ocurrió un error mostrando los resultados: ${esc(err.message || String(err))}. Los datos sí se procesaron — revisa "Indicadores" o "Pendientes" en el menú, y avisa para corregir esta vista.</div>`;
    }
  }
  ...
  function panelCargaHtml(tipo) {
    ...
    if (bloqueado) {
      return `
        <div class="panel panel-bloqueado">
          <div class="panel-header">
            <div>
              <h3>${TITULOS[tipo].archivo}</h3>
              <p>${TITULOS[tipo].sub}</p>
            </div>
            <span class="badge muted">Se habilita al terminar el paso 1</span>
          </div>
          <div class="panel-body">
            <div class="locked-note">
  ...
  function panelUniversoHtml(datosPendientes, datosTerminadas) {
    ...
    return `
      ...
      ${panelIndicadoresBaseHtml(datosPendientes, datosTerminadas, bases)}
  ...
  /** Tabla de indicadores por base: terminados, pendientes, total de
   * solicitudes recibidas y % de entrega (terminados/total), coloreado de
   * rojo (0%) a verde (100%) según el nivel de alcance. */
  function panelIndicadoresBaseHtml(datosPendientes, datosTerminadas, bases) {
    if (bases.length === 0) return "";

    const filas = bases
      .map((b) => {
        const terminados = datosTerminadas.filter((f) => f.base === b).length;
        const pendientes = datosPendientes.filter((f) => f.base === b).length;
        const total = terminados + pendientes;
        const pct = total > 0 ? Math.round((terminados / total) * 100) : null;
        return { base: b, terminados, pendientes, total, pct };
      })
      .sort((a, b) => (a.pct ?? 101) - (b.pct ?? 101)); // primero las bases con menor % de entrega

    const totalTerminados = datosTerminadas.length;
    const totalPendientes = datosPendientes.length;
    const totalGeneral = totalTerminados + totalPendientes;
    const pctGeneral = totalGeneral > 0 ? Math.round((totalTerminados / totalGeneral) * 100) : null;
            ...
            <table class="data indicadores-table">
              ...
              <tbody>
                ${filas
                  ...
                  <tr>
                    <td>${esc(f.base)}</td>
                    <td class="tabular">${f.terminados}</td>
                    <td class="tabular">${f.pendientes}</td>
              ...
              <tfoot>
                <tr class="fila-total">
                  <td>Total general</td>
                  <td class="tabular">${totalTerminados}</td>
                  <td class="tabular">${totalPendientes}</td>
  ...
  async function publicarEnStore() {
    ...
    actualizarDatosWF({
      pendientes: combinado.pendientes,
      terminadas: combinado.terminadas,
      lista: [...combinado.pendientes, ...combinado.terminadas],
    });
  }
}
