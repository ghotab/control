...
function calcularVideoYArea(registroST, registroMON) {
  ...
  if (tabListo) return { hayVideo: "Sí", areaAtencion: "TAB" };

  const monListo = registroMON && registroMON.videos?.length > 0 && pareceVideoCompartido(registroMON.observacionesAbordo);
  if (monListo) return { hayVideo: "Sí", areaAtencion: "MONITOREO" };

  return { hayVideo: "No", areaAtencion: "—" };
}
...
export function calcularListaCompacta(datosPendientes, datosTerminadas, datosMonitoreo) {
  ...
  const marcar = (arr, estatus) =>
    (arr || []).map((r) => {
      ...
      const { hayVideo, areaAtencion } = calcularVideoYArea(r, mon);
      return { ...r, estatus, hayVideo, areaAtencion };
    });
