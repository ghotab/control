...
function montarVistaBase(root, baseUsuario) {
  ...
  async function manejarArchivoPrincipal(file) {
    ...
    try {
      ...
      if (!nombreHoja) throw new Error(`No encontré la hoja "BA Soporte Técnico GHO-Gestión" (ni su equivalente ASJ) en este archivo.`);
  ...
  async function manejarArchivoSiniestro(file) {
    ...
    try {
      ...
      if (!nombreHoja) throw new Error(`No encontré la hoja "GHO BA Registro del Siniestro" (ni su equivalente ASJ) en este archivo.`);
      const indice = {};
...
function montarVistaJefe(root) {
  ...
  async function procesarUniverso(tipo, registros) {
    const st = estado[tipo];
    try {
      const { filas } = await enriquecerRegistros(registros);
      st.filas = filas;
    } catch (err) {
      console.error(err);
